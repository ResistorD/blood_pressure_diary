...
class RiskGate:
    ...
    def check_market(self, market_id: str) -> GateVerdict:
        ...
        try:
            ...
            if ks:
                return GateVerdict(False, "BLOCKED", "KILL", "kill-switch включён: новые paper-открытия запрещены")
        except Exception:
        ...
        if rc:
            return GateVerdict(False, "BLOCKED", "RISK", rc["explain_short"] or "risk constraint")
        ...
        if dq:
            return GateVerdict(False, "BLOCKED", "DATA", dq["explain_short"] or "data quality")
    ...
    def _check_limits(self, market_id: str) -> Optional[GateVerdict]:
        ...
        if max_pos and st.get("open_positions", 0) >= max_pos:
            return GateVerdict(False, "BLOCKED", "LIMIT", "слишком много открытых paper-позиций")
        ...
        if max_total and notional_open >= max_total:
            return GateVerdict(False, "BLOCKED", "LIMIT", "исчерпан общий лимит капитала (paper)")
        ...
        try:
            ...
            if gk:
                ...
                if max_group and float(per_group.get(gk, 0.0) or 0.0) >= max_group:
                    return GateVerdict(False, "BLOCKED", "LIMIT", "исчерпан лимит экспозиции по кластеру")
        except Exception:
        ...
        if self.repo.paper_has_open_position(market_id):
            return GateVerdict(False, "BLOCKED", "LIMIT", "уже есть открытая paper-позиция по рынку")
